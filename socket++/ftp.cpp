...
// host if the std::ostream o is set, or it sends a file to the remote
// host if the std::istream i is set.
ftp::replycodea ftp::ftpbuf::ftpdata (int portno, std::istream* i, std::ostream* o,
...
ftp::ftp (std::ostream* out)
: std::ios (0)
{
  std::ios::init (new ftpbuf (out));
}
...
ftp::replycodea ftp::ftpbuf::useraddr (sockinetaddr sa)
{
  if (sa.sin_addr.s_addr == 0) {
    ...
    if (::gethostname (hostname, 63) == -1) throw err(EADDRNOTAVAIL, "ftp::replycodea");
    hostent* hp = gethostbyname(hostname);

    if (hp == 0) throw err(EADDRNOTAVAIL, "ftp::replycodea");
