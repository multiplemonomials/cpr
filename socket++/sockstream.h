...
public:
 ...
 //since there are different addresses for different kinds of sockets
class sockAddr 
{
public:
  virtual		~sockAddr() {}
...
{
  //if on Windows, calls WSAStartup() if necessary.
  //otherwise, does nothing.
  void initWSA();
public:


  //wsa startup data struct from WSAStartup()
  //must be initialized before WSA sockets are used
#ifdef USE_WINSOCK
  static std::shared_ptr<WSADATA> wsaStartupInfo;
#endif
...
public:
  ...
  //create from existing OS socket
  sockbuf (const sockdesc& sd);

  //create a new OS socket
  sockbuf (int domain, type, int proto);

  //assign data of sockbuf
